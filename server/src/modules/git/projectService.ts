...
const summaryFor = (record: ProjectRecord, workspace: WorkspaceState, cached = false): RepositorySummary => {
  ...
  return {
    ...
    syncMessage: "Connect GitHub to compare this workspace with its remote branch.",
...
export const projectService = {
  ...
  markPushed(roomId: string, workspace: WorkspaceState, sha: string, message: string, authorName = "Code Collaborator") { const record = projects.get(keyFor(roomId, workspace.id)); if (!record) throw new Error("Project is not initialized"); const changedFiles = calculateDiff(record, workspace).filter((entry) => entry.staged).map((entry) => entry.path).slice(0, 50); record.baseFiles = new Map([...workspaceFilesByPath(workspace)].filter(([path]) => !sensitivePath.test(path)).map(([path, file]) => [path, file.content])); record.baseHead = sha; record.staged.clear(); record.pendingCommit = undefined; record.history = [{ sha, message, authorName, authoredAt: Date.now(), changedFiles }, ...record.history].slice(0, MAX_HISTORY); record.project = { ...record.project, updatedAt: Date.now() }; return record; },
