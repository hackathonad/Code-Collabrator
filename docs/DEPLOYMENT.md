...
### Improvement Batch 4 acceptance

After deploying the current frontend and backend, open a room and confirm Explorer can load the read-only project snapshot. Verify that entry points/areas come from the imported virtual workspace, fixed checks show **not run** until they actually run, Git shows unavailable until a repository is connected, and AI shows unavailable when no server-side model is configured. Use Understand project, Tour, What’s happening?, Session summary, and Handoff to prepare prompts in the existing assistant; no prompt should claim evidence that the bounded context did not inspect.

Create two guest collaborators and start an AI task. Confirm the shared task remains visible through Socket.IO, priority is only Normal/High/Urgent presentation metadata, assignment accepts only current room collaborators, watchers are bounded, and activity filters separate Humans, AI, Git, and Tests. Change a file before approving an AI patch and confirm the existing stale/conflict path still wins. Run a fixed validation and review the actual Git diff before any explicit commit or push action.

The Batch 4 project experience is read-only and process-memory backed for task/project summaries. It does not require a Supabase migration, Supabase Auth, new browser variables, a deployment provider connection, or a new server instance. Automatic deploy, arbitrary shell execution, vision uploads, durable AI task recovery, and multi-instance Socket.IO fan-out remain intentionally unsupported or deferred; the UI must show those states honestly.
