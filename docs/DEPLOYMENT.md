...
The client can request cancellation for an active task. Reconnects receive room-scoped task and proposal metadata through Socket.IO; proposal history never contains replacement code, expected content, or previews. Room deletion clears in-memory agent indexes, tasks, and proposal state. These controls do not add authentication or change the database-only Supabase architecture.
