...
Validation tools use fixed npm categories (`typecheck`, `lint`, `tests`, `build`), `shell: false`, bounded output, and a 30-second limit. Results are normalized as not-run, running, passed, failed, skipped, unavailable, or cancelled, including cancellation and timeout outcomes. The child process environment removes variables whose names indicate API keys, secrets, tokens, passwords, Supabase, or guest-session credentials. Validation is an explicit user action and does not auto-apply a proposal. Do not add provider credentials to client variables or include secrets in workspace files.
