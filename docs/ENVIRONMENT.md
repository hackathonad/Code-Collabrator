...
The Phase 6F index cache is limited to 100 room/workspace entries and invalidates on content or workspace-structure changes. Each index is capped at 500 files, 20 ranked relevant files, and bounded symbols/imports/summaries. Debug diagnoses are limited to eight hypotheses per response with five short evidence items each. Active-agent cancellation is server-aware, while pre-cancelled validation exits before creating a child process.
