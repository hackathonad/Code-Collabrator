...
export const SourceControlPanel = ({ roomId, session, repository, loading, error, onRefresh, onNotify }: SourceControlPanelProps) => {
  ...
  return <section className="shrink-0 border-t border-[var(--border)] px-3 pt-3 pb-3">
    ...
    {!loading && !error && metadata ? <div className="mt-2 grid gap-2 text-xs">
      ...
      <div className="flex items-center justify-between"><span className="font-semibold text-[var(--text-secondary)]">Changes</span><button type="button" onClick={() => setShowDiff((value) => !value)} className="text-[11px] text-[var(--accent)]">{showDiff ? "Hide diff" : "View diff"}</button></div>
