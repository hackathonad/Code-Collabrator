...
export const HomePage = ({ guestMode = false }: { guestMode?: boolean }) => {
  ...
  const rejoinRecentRoom = async (recentRoom: RecentRoom) => {
    ...
    try {
      ...
      const room = await api.getRoom(recentRoom.roomId, existingSession);
      if (existingSession) {
          ...
          <div className="theme-panel motion-reveal motion-surface rounded-[32px] border p-6 shadow-panel backdrop-blur-xl">
            ...
            <div className="mt-5 grid gap-3">
              ...
              <div className="theme-surface rounded-2xl border p-4">
                ...
                <p className="mt-1 text-sm theme-text-muted">A focused shared workspace with chat, presence, and optional tools when configured.</p>
              </div>
            </div>
          </div>
        </section>
